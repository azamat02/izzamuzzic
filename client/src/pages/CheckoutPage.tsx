      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <main className="min-h-screen bg-[#0a0a0a] pt-32 pb-24">
          <div className="max-w-2xl mx-auto px-4 text-center py-20">
            ...
            <Link to="/merch" className="inline-block bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium">
              Перейти в магазин
            </Link>
          </div>
        </main>
            ...
            {[1, 2, 3, 4].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                  s <= step ? 'bg-[var(--color-accent)] text-white' : 'bg-[#1a1a1a] text-[#555]'
                }`}>
                  {s}
                </div>
                {s < 4 && <div className={`w-8 h-0.5 ${s < step ? 'bg-[var(--color-accent)]' : 'bg-[#1a1a1a]'}`} />}
              </div>
            ))}
          </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#141414] border border-[#1a1a1a] rounded-lg px-4 py-3 text-white focus:border-[var(--color-accent)] focus:outline-none"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-[#141414] border border-[#1a1a1a] rounded-lg px-4 py-3 text-white focus:border-[var(--color-accent)] focus:outline-none resize-none"
              ...
              <button
                onClick={() => setStep(2)}
                disabled={!step1Valid}
                className="w-full bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium tracking-wider disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Далее
              </button>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div className="bg-[#141414] border border-[#1a1a1a] rounded-lg p-5 space-y-3">
                ...
                <div className="border-t border-[#1a1a1a] pt-3 flex justify-between">
                  ...
                  <span className="text-[var(--color-accent)] font-bold text-lg">{totalAmount.toLocaleString()} KZT</span>
                </div>
              </div>
              ...
              <div className="bg-[#1a1a1a] border border-[#333] rounded-lg p-5 text-center space-y-4">
                ...
                {kaspiPayUrl ? (
                  <a
                    ...
                    className="inline-block bg-[#f14635] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium"
                  >
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setStep(3)} className="flex-1 bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium">
                  Далее — загрузить чек
                </button>
              </div>
            </motion.div>
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div
                ...
                onDrop={handleDrop}
                className="border-2 border-dashed border-[#333] rounded-lg p-10 text-center hover:border-[var(--color-accent)] transition-colors"
              >
                {receiptPreview ? (
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium disabled:opacity-40 disabled:cursor-not-allowed"
                >
          ...
          {step === 4 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10 space-y-6">
              ...
              <div className="flex gap-3 justify-center">
                <Link
                  to="/merch"
                  className="inline-block bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg hover:bg-[var(--color-accent-light)] transition-colors font-medium"
                >
                  Вернуться в магазин
                </Link>
              </div>
